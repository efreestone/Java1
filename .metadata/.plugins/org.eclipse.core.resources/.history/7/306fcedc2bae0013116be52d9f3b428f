...
public class CitiesJSON {
	...
	public static String readJSON(String selectedCity) {
		...
		try {
			...
			city = newObject.getJSONObject("cities")
					.getJSONObject(selectedCity).getString("cityName");
			state = newObject.getJSONObject("cities")
					.getJSONObject(selectedCity).getString("stateName");
			fahrenheit = newObject.getJSONObject("cities")
					.getJSONObject(selectedCity).getString("fahrenheitTemp");
			celsius = newObject.getJSONObject("cities")
					.getJSONObject(selectedCity).getString("celsiusTemp");

			// Set tempSelected based on selected temp in MainActivity. Defaults
			// to Fahrenheit if nothing is selected
			if (MainActivity.selectedTempMeasurement
					.equalsIgnoreCase("Fahrenheit")) {
